...
Flowo is actively developed in scoped, additive batches — new features ship without breaking what already works. The project is private during this build phase; the live deployment link above always reflects the current version.

## License

All Rights Reserved. See `LICENSE.md` for details. The live application may be viewed and used via the deployed link; the source code is not licensed for reuse or redistribution.
